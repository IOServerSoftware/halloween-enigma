...
bool is_ready_first(const dpp::message_create_t& event) {
    if (event.msg.content.substr(prefix.length()) == "ready") return true;
    if (event.msg.content.substr(prefix.length()) == "Ready") return true;
    if (event.msg.content.substr(prefix.length()) == "READY") return true;
    if (event.msg.content.substr(prefix.length()) == "I am ready") return true;
    if (event.msg.content.substr(prefix.length()) == "i am ready") return true;
    if (event.msg.content.substr(prefix.length()) == "I AM READY") return true;
    if (event.msg.content.substr(prefix.length()) == "i'm ready") return true;
    if (event.msg.content.substr(prefix.length()) == "I'm ready") return true;
    if (event.msg.content.substr(prefix.length()) == "I'M READY") return true;
    return false;
}
...
void nothing_is_what_it_seems(dpp::cluster& bot, const dpp::message_create_t& event, std::string& user_progress_path) {
    ...
    if (current == 4 && is_ready_first(event)) {
